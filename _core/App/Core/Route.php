...
class Route
{
    ...
    function getPost($route,$action,$param=false)
    {
        $route = $this->route($route);
        if($_SERVER['REQUEST_METHOD'] == 'POST' && $route == $this->route) {
            $this->run($action,$param);
    ...
    function crud($controller) {
        switch ($this->route) {
            case '':
                ...
                $this->post('',ucfirst($controller).'Controller@index');
                break;
            case 'listar':
                $this->getPost('listar',ucfirst($controller).'Controller@listar','id');
                break;
            case 'novo':
    ...
    function redireciona($route)
    {
        header('location: ' . URL_ADMIN . '/../'.$route);
        exit();
    }

    private function run($action,$paramData) {
        global $db, $banco, $pdo, $id_usuario, $response, $param;
        $param = $paramData;
        $parts = explode(',',$param);
